...
public:
	...
	*/
	...
	{
		if (m->message == WM_HOTKEY && m->wParam == id_ && id_ == 100) {

			emit trigger_->activated();
			return true;
		}

		else if (m->message == WM_HOTKEY && m->wParam == id_ && id_ == 200)
		{


			DWORD nowtime = ::GetTickCount();

			if ( nowtime - hotkeytime <  ::GetDoubleClickTime())
			{
				hotkeytime = 0;
				emit trigger_->activated();
			}
...
DWORD GlobalShortcutManager::KeyTrigger::Impl::hotkeytime = 0;
