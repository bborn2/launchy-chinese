	...
	directoryItemDelegate(this, FileBrowser::Directory)
{
	...
	actionKeys <<  tr("Double") << tr("Space") << tr("Tab") << tr("Caps Lock") << tr("Backspace") << tr("Enter") << tr("Esc") <<
	...
	iActionKeys << Qt::Key_F16 << Qt::Key_Space << Qt::Key_Tab << Qt::Key_CapsLock << Qt::Key_Backspace << Qt::Key_Enter << Qt::Key_Escape <<
...
void OptionsDialog::accept()
{
	...
	if ( (iMetaKeys[genModifierBox->currentIndex()] != Qt::AltModifier &&
		iMetaKeys[genModifierBox->currentIndex()] != Qt::ControlModifier &&
		iMetaKeys[genModifierBox->currentIndex()] != Qt::ShiftModifier)
		&&
		iActionKeys[genKeyBox->currentIndex()] == Qt::Key_F16)
	{
		QMessageBox box(this);
		box.setText(tr("select another hotkey for double tap (ctrl alt or shift)"));
		box.exec();
		return;
	}
